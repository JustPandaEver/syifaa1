...
const starts = async (Syifa = new WAConnection()) => {
    ...
    Syifa.browserDescription = ['RAKUN SHOP GAME','Chrome', '3.0']
    console.log(banner)
    Syifa.on('qr', () => {
        console.log(color('[SCAN ME]','red'), color('AYOK CEPAT DI SEKEN🔍'))
    })
...
if (anu.action == 'add' && !mem.includes(Syifa.user.jid)) {
			 ...
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                teks = `Hallo Bero Ku @${num.split('@')[0]}, \n┌Nomor: *${num.split('@')[0]}*\n├Bɪᴏ* : *${thu.status}*\n└Mᴇᴍʙᴇʀs : ${memeg}*\n\n Selamat Datang di ${mdata.subject}`
                tekss = `JAM : ${time_wel} - TGL : ${time_welc}`
                welcomeBut = [{buttonId:`#intro`,buttonText:{displayText:'WELCOME BOS KU 👋'},type:1}]
            ...
            if (anu.action == 'remove' && !mem.includes(Syifa.user.jid)) {
                ...
                out = `*KOK KELUAR KENAK MENTAL YA 😆.... KASIAN* 👋 @${num.split('@')[0]}`
                goodbyeBut = [{buttonId:` `,buttonText:{displayText:'SELAMAT TINGGAL BEBAN 😆'}, type:1}]
                goodbyeButt = { contentText: `${out}`, footerText: `1 BEBAN KELUAR. HADEH... KASIAN SI BEBAN?`, buttons: goodbyeBut, headerType: 6, locationMessage: bosco3.message.locationMessage}
